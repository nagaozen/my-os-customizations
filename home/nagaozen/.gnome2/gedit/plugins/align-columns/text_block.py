# Copyright (c) 2011 Hugo Henriques Maia Vieira, (c) 2012 Fabio Zendhi Nagao
#
...
class TextBlock(object):
    ...
    def get_columns_number(self):
        ...
        sep = clipboard.wait_for_text()

        seps_number = self.lines_str[0].count(sep)
        for line in self.lines_str:
            if line.count(sep) != seps_number:
                raise DifferentNumberOfColumnsError
        columns_number = seps_number + 1
    ...
    def align(self):
        ...
        for line in self.lines_list:
            ...
            for index in range(len(self.columns)):
                text += line[index] + (self.columns[index] - len(line[index]))*' ' + ' ' + sep + ' '
            text = text[:-3].rstrip() + '\n'
            i+=1
