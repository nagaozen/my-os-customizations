...
class MacrosWindowHelper:
	...
	def install_ui(self):
		...
		self._action_group = gtk.ActionGroup("PlumaMacrosPluginActions")
	...
	def on_start_recording(self, action):
		handlers = []
#		handlers.append( self._window.connect("event", self.on_event) )
		handlers.append( self._window.get_active_view().connect("event", self.on_event) )
	...
	def on_event(self, window, event):
		self._macro.append(event.copy())

class MacrosPlugin(pluma.Plugin):
